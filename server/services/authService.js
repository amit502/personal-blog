...
exports.token = async (req, res) => {
    try {
        ...
        if(AuthHelper.refreshTokenDecode(user.refreshToken)?.id 
        !== AuthHelper.refreshTokenDecode(refreshToken)?.id) return res.sendStatus(403);

        jwt.verify(refreshToken, process.env.REFRESH_TOKEN_SECRET, (err,userr) => {
...
exports.signUp = async (data,res) => {
    try {
        const userExists = await db.User.findOne({where: {email:{[Op.eq]: data.email}}});
        if(userExists) {
            res.status(409);
            return res.json({ message: 'User already exists! Try using different email.'});
        }
        let postData = data;
