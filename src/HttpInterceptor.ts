...
export const HttpRequestInterceptor = axios.interceptors.request.use(
  (config) => {
    const token = JSON.parse(localStorage.getItem('blogUser') || 'null')?.accessToken;
  ...
  function (error) {
    ...
    if ((error.response.status === 403 || error.response.status === 401) && !originalRequest._retry) {
      originalRequest._retry = true;
       const refreshToken = JSON.parse(localStorage.getItem('blogUser') || 'null')?.refreshToken;
 
      return getAccessToken(refreshToken)
        .then((res) => {
          if (res) {
            let usr = JSON.parse(localStorage.getItem('blogUser') || 'null');
            usr.accessToken = res?.data?.accessToken;
            localStorage.setItem('blogUser', JSON.stringify(usr || 'null'));
